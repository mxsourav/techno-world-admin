...
export default function PromotionEditModal({ promotion, onClose, onSuccess }: PromotionEditModalProps) {
  ...
  useEffect(() => {
    if (promotion && promotion.id) {
      ...
      let draft: any = null;
      try {
        const saved = localStorage.getItem(PROMO_DRAFT_KEY);
        if (saved) draft = JSON.parse(saved);
      } catch (e) {}

      if (draft && typeof draft === 'object' && (draft.name || draft.code || draft.discountValue)) {
        setFormData(draft);
        setHasDraft(true);
        toast.info('Restored your auto-saved promotion draft');
      } else {
        const now = new Date();
  ...
  useEffect(() => {
    if (!promotion?.id) {
      if (formData.name || formData.code || formData.discountValue > 0) {
        try {
          localStorage.setItem(PROMO_DRAFT_KEY, JSON.stringify(formData));
          setHasDraft(true);
        } catch (e) {}
      }
    }
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {hasDraft && !promotion?.id && (
                <span className="text-[9px] font-extrabold uppercase tracking-wider text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-950/50 border border-emerald-300 dark:border-emerald-700/40 px-2 py-0.5 rounded-full flex items-center gap-1">
                  <Check className="h-3 w-3" /> Auto-Saved Draft
                </span>
              )}
            </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col lg:flex-row relative bg-slate-50/50 dark:bg-[#080d1a]">
          ...
          <div className={`flex-1 overflow-y-auto p-3.5 sm:p-4 ${activeTab === 'preview' ? 'hidden lg:block' : 'block'}`}>
            <div className="max-w-2xl mx-auto space-y-3">
              ...
              {/* Promotion Type */}
              <div>
                <label className="block text-xs font-bold text-slate-900 dark:text-white mb-1.5">Promotion Type</label>
              ...
              <div className="bg-white dark:bg-[#0d1324]/80 rounded-xl border border-slate-200 dark:border-white/[0.08] overflow-hidden shadow-sm divide-y divide-slate-100 dark:divide-white/[0.06]">
                ...
                <div>
                  ...
                  {expandedSection === 'product' && (
                    <div className="p-3.5 bg-slate-50/50 dark:bg-white/[0.02] space-y-2.5 border-t border-slate-100 dark:border-white/[0.06]">
                      <div>
                        <label className="block text-xs font-bold text-slate-700 dark:text-neutral-300 mb-1.5">Target Specific Categories</label>
                          ...
                          <button
                            ...
                            className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all border ${
                              formData.allowedCategories.length === 0
                                ...
                                : 'bg-white dark:bg-white/[0.06] text-slate-700 dark:text-neutral-300 border-slate-200 dark:border-white/10 hover:border-slate-300'
                            }`}
                          >
                            ...
                            return (
                              <button
                                ...
                                className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all border flex items-center gap-1.5 ${
                                  isSelected
                                    ? 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border-emerald-300 dark:border-emerald-600/50 ring-1 ring-emerald-400'
                                    : 'bg-white dark:bg-white/[0.06] text-slate-600 dark:text-neutral-300 border-slate-200 dark:border-white/10 hover:border-slate-300'
                                }`}
                              >
